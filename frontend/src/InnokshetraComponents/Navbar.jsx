...
const Navbar = () => {
  ...
  const navItems = [
    ...
    { id: 2, text: 'GLANCE', sectionId: 'glance' },
    { id: 3, text: 'TIMELINE', sectionId: 'timeline' },
  ...
  return (
    <div className='  w-full  flex justify-evenly items-center h-20 mx-auto  text-white z-50 fixed bg-black'>
      ...
      {/* Social Icons */}
      <ul className='hidden   md:flex border-white rounded-lg  p-0 social-icons'>
  {socialIcons.map(iconObj => (
    ...
    >
      ...
      <ul
        className={
          nav
           ? ' z-10 fixed md:hidden left-0 top-0 w-[60%]  border-r border-r-gray-900   ease-in-out duration-500 backdrop-blur-3xl '
            : 'ease-in-out w-[60%] duration-500 fixed top-0 bottom-0 left-[-100%]'
        }
      >
        {/* Mobile Logo */}
        <h1 className='w-full text-2xl font-bold text-white m-4 '>Innokshetra</h1>

        {/* Mobile Navigation Items */}
            {navItems.map(item => (
        <li
          key={item.id}
          className='p-4 hover:border-4 hover:border-white  hover:text-white border-b rounded-xlduration-300 hover:text-black cursor-pointer border-gray-600  backdrop-blur-3xl'
